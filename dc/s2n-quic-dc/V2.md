...
The existing transport implementation uses a byte stream abstraction built on top of UDP sockets. Each RPC request creates a new stream with its own derived encryption key. The system uses a worker-based architecture where send and receive operations are handled by separate worker tasks that poll sockets. Each stream spawns two dedicated workers - one for sending and one for receiving.
...
This underlying transport should be abstracted from the application interface. For environments that support it, libfabric+EFA will be used for accelerated networking capabilities. For environments without strong libfabric support, a UDP-based protocol is used. AF_XDP will be the primary socket interface used in production. The system must support macOS for developer workstation environments by falling back to system sockets. Integration with the Bach simulator is required for deterministic testing scenarios.
...
The endpoint serves as the single entry point for all application transfer requests. It manages worker lifecycle and provides transfer APIs that applications use to initiate communication with peers.

Rather than forcing applications to work with a generic byte stream abstraction, the endpoint exposes distinct methods for each transfer type: unary RPC, streaming response, streaming request, bidirectional, and raw bulk transfer. It also exposes APIs for allocating and encrypting messages. Each message accepts a priority, giving applications explicit control over scheduling behavior. Messages can be cheaply cloned and sent across multiple streams.

The endpoint must be cheaply cloneable to support concurrent access from multiple application threads. This is achieved through Arc-based sharing of underlying state. Thread safety is essential since many application threads will simultaneously initiate transfers. The endpoint minimizes allocation per transfer request to maintain high throughput under burst load conditions.

#### Application-Worker Communication

The endpoint communicates with workers through ring buffers with separate queues per priority level. This enables workers to process higher-priority items first. Applications allocate and associate their own ring buffers for responses, creating a bidirectional channel with minimal synchronization and maximum allocation reuse.
...
The message router maps incoming messages to appropriate NIC workers based on peer address and current load conditions. The router enables bandwidth scaling across available NICs while maintaining ordering guarantees where necessary.

Lock-free or minimal-locking design is essential because the router sits in the hot path for all messages. With tens of thousands of concurrent transfers, contention on routing decisions would create a bottleneck. The router uses flow hashing to assign peers to workers, ensuring that all transfers to a given peer stream use the same worker. This maintains causal ordering within a stream without requiring cross-worker coordination.
...
The transport handles per-flow bandwidth limits differently based on the underlying transport mechanism. The system uses port randomization to work around per-flow limits by allocating multiple ports and spreading bulk data transmission across them. This enables aggregating individual flow bandwidths up to the total NIC capacity. For instance, using 5 ports at 5 Gbps each achieves the 25 Gbps NIC limit.
...
The system uses 8 priority levels (`0..=7`). Highest-priority messages (lower valued) are always transmitted before lower priority messages (higher valued). This ensures the system consistently clears out work rather than adding delays to processing lower priority messages. Work queues are kept shallow. It does mean that high priority messages can starve low priority messages, so applications will need to be careful with priority assignments.
...
The single-producer single-consumer nature of the timing wheel within a worker eliminates synchronization overhead. Only the worker thread inserts and removes events, so no atomic operations or locks are required.

### Priority Queue

Each worker maintains a priority queue of messages ready for transmission. The queue orders messages by priority level, with higher priority messages scheduled before lower priority ones.

This component directly addresses the priority-based scheduling requirement. Applications need control over which messages receive bandwidth when resources are constrained. Without priority scheduling, all messages receive equal treatment regardless of importance.

The queue contains 8 priority levels, from 0 (highest) to 8 (lowest). Each level maintains a FIFO queue of messages at that priority. When selecting the next message to schedule, the worker scans from highest to lowest priority and selects the first available message. This provides deterministic priority ordering while maintaining fairness within each priority level.
