...
Transfer notification messages differentiate between transfer patterns to enable proper handling on the receiver side. The message type indicates whether this is initiating a new unary request, initiating a new streaming request, or appending an item to an existing stream.

`TRANSFER_NOTIFY` messages announce pending transfers to the receiver. The message encodes a list of transfers, with each transfer including: transfer identifier, transfer pattern type (unary, streaming request, streaming response, bidirectional, bulk), optional stream identifier (for stream appends), peer causality token, payload size, priority level, application metadata for RPC handler dispatch, ephemeral data key, buffer ID, and optional inline payload for small transfers.

The transfer pattern type field indicates the nature of the transfer. For unary requests, this is a standalone single request-response exchange. For streaming patterns, the type distinguishes between initiating a new stream versus appending to an existing stream. When appending to an existing stream, the stream identifier links the new transfer notification to the previously-established stream context. This enables the receiver to route stream items to the correct handler and maintain ordering within the stream while allowing concurrent processing across different streams.
