...
fn enable_busy_poll(socket: &UdpSocket, busy_poll_us: u32) -> std::io::Result<()> {
    ...
    if res != 0 {
        eprintln!(
            "Warning: Failed to set SO_BUSY_POLL: {}",
    ...
    if res != 0 {
        eprintln!(
            "Warning: Failed to set SO_PREFER_BUSY_POLL: {}",
            std::io::Error::last_os_error()
        );
    }
...
async fn run_client(
    ...
    println!(
        "Burst size: {} packets, Delay: {}ms, Payload: {} bytes",
    ...
    for burst_num in 0..num_bursts {
        ...
        while burst_received < burst_sent && recv_start.elapsed() < timeout {
            ...
            match tokio::time::timeout(Duration::from_millis(10), socket.recv_from(&mut recv_buf))
                .await
            {
                Ok(Ok((len, _))) => {
                    if len >= 8 {
    ...
    if total_received > 0 {
        ...
        println!(
            "RTT: min={}µs, max={}µs, avg={}µs",
            min_rtt_us, max_rtt_us, avg_rtt_us
        );
    }
