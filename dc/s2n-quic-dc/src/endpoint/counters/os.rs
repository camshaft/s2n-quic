...
//!
//! This module periodically snapshots `/proc` network stats, computes deltas, and
//! emits those deltas into the shared `counter::Registry`. Consumers can either
//! call [`Collector::record_delta`] directly on their own schedule or use
//! [`spawn`] when Tokio is available.

#[cfg(target_os = "linux")]
use std::collections::HashMap;

/// Spawns a periodic Linux OS-metrics collector.
///
/// The collector reads from procfs once per second and records per-interval
/// deltas into the provided counter registry.
#[cfg(all(feature = "tokio", target_os = "linux"))]
pub fn spawn(counters: crate::counter::Registry) {
...
#[cfg(not(all(feature = "tokio", target_os = "linux")))]
pub fn spawn(_counters: crate::counter::Registry) {}
...
impl Collector {
    ...
    /// Reads a fresh snapshot and records monotonic deltas into counters.
    pub fn record_delta(&mut self) {
        let current = Snapshot::read();
        let mut delta = HashMap::new();
        for (key, value) in &current.values {
            let prev_value = self.prev.values.get(key).copied().unwrap_or_default();
...
#[cfg(target_os = "linux")]
fn collect_udp_table_sums(out: &mut HashMap<String, u64>, path: &str, prefix: &str) {
    let Ok(content) = std::fs::read_to_string(path) else {
        return;
    };
    let mut lines = content.lines();
