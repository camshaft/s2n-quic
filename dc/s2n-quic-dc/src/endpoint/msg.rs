...
pub enum Stream {
    ...
    ///   allocation (`message_size`) won't fit the currently-advertised window. Here the demand is
    ///   *known and bounded*: exactly `desired_offset`, the end of one message segment (≤ the max
    ///   segment size). The reader must open the window to that specific offset so the segment can
    ///   complete — independent of the streaming `growth_ratio` ramp, which would be both too slow
    ///   (a stalled segment delivers nothing, so consumption never advances to pace doublings) and
    ///   semantically wrong (this isn't open-ended streaming demand).
    Blocked {
