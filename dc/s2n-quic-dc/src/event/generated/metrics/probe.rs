    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                185usize => Self(stream_packet_abandoned),
                187usize => Self(stream_packet_abandoned__payload_len__total),
                192usize => Self(stream_packet_spuriously_retransmitted),
                194usize => Self(stream_packet_spuriously_retransmitted__payload_len__total),
                198usize => Self(stream_max_data_received),
                199usize => Self(stream_max_data_received__increase__total),
                201usize => Self(stream_data_blocked_transmitted),
                202usize => Self(stream_data_blocked_received),
                203usize => Self(stream_control_packet_transmitted),
                206usize => Self(stream_control_packet_received),
                210usize => Self(stream_receiver_errored),
                211usize => Self(stream_sender_errored),
                212usize => Self(connection_closed),
                213usize => Self(endpoint_initialized),
                218usize => Self(endpoint_udp_immediate_transmission_scheduled),
                223usize => Self(endpoint_udp_transmission_scheduled),
                229usize => Self(endpoint_udp_transmission_rejected),
                236usize => Self(endpoint_udp_packet_transmitted),
                241usize => Self(endpoint_udp_transmit_errored),
                246usize => Self(endpoint_udp_packet_received),
                251usize => Self(endpoint_udp_receive_errored),
                252usize => Self(path_secret_map_initialized),
                254usize => Self(path_secret_map_uninitialized),
                258usize => Self(path_secret_map_background_handshake_requested),
                260usize => Self(path_secret_map_entry_inserted),
                262usize => Self(path_secret_map_entry_ready),
                264usize => Self(path_secret_map_entry_replaced),
                266usize => Self(path_secret_map_id_entry_evicted),
                269usize => Self(path_secret_map_address_entry_evicted),
                272usize => Self(unknown_path_secret_packet_sent),
                274usize => Self(unknown_path_secret_packet_received),
                276usize => Self(unknown_path_secret_packet_accepted),
                278usize => Self(unknown_path_secret_packet_rejected),
                280usize => Self(unknown_path_secret_packet_dropped),
                282usize => Self(key_accepted),
                285usize => Self(replay_definitely_detected),
                286usize => Self(replay_potentially_detected),
                288usize => Self(replay_detected_packet_sent),
                290usize => Self(replay_detected_packet_received),
                292usize => Self(replay_detected_packet_accepted),
                294usize => Self(replay_detected_packet_rejected),
                296usize => Self(replay_detected_packet_dropped),
                298usize => Self(stale_key_packet_sent),
                300usize => Self(stale_key_packet_received),
                302usize => Self(stale_key_packet_accepted),
                304usize => Self(stale_key_packet_rejected),
                306usize => Self(stale_key_packet_dropped),
                308usize => Self(path_secret_map_address_cache_accessed),
                311usize => Self(path_secret_map_address_cache_accessed_hit),
                314usize => Self(path_secret_map_id_cache_accessed),
                316usize => Self(path_secret_map_id_cache_accessed_hit),
                318usize => Self(path_secret_map_cleaner_cycled),
                335usize => Self(path_secret_map_id_write_lock),
                338usize => Self(path_secret_map_address_write_lock),
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info) -> Self {
                match info.id {
                    ...
                    191usize => Self(stream_packet_abandoned__retransmission),
                    197usize => Self(stream_packet_spuriously_retransmitted__retransmission),
                    209usize => Self(stream_control_packet_received__authenticated),
                    216usize => Self(endpoint_initialized__tcp),
                    217usize => Self(endpoint_initialized__udp),
                    310usize => Self(path_secret_map_address_cache_accessed__hit),
                    315usize => Self(path_secret_map_id_cache_accessed__hit),
                    _ => unreachable!("invalid info: {info:?}"),
                }
            }
        }
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info, _variant: &'static info::Variant) -> Self {
                match info.id {
                    ...
                    214usize => Self(endpoint_initialized__acceptor__protocol),
                    215usize => Self(endpoint_initialized__handshake__protocol),
                    219usize => Self(endpoint_udp_immediate_transmission_scheduled__peer__protocol),
                    224usize => Self(endpoint_udp_transmission_scheduled__peer__protocol),
                    230usize => Self(endpoint_udp_transmission_rejected__peer__protocol),
                    237usize => Self(endpoint_udp_packet_transmitted__peer__protocol),
                    242usize => Self(endpoint_udp_transmit_errored__peer__protocol),
                    247usize => Self(endpoint_udp_packet_received__peer__protocol),
                    259usize => {
                        Self(path_secret_map_background_handshake_requested__peer_address__protocol)
                    }
                    261usize => Self(path_secret_map_entry_inserted__peer_address__protocol),
                    263usize => Self(path_secret_map_entry_ready__peer_address__protocol),
                    265usize => Self(path_secret_map_entry_replaced__peer_address__protocol),
                    267usize => Self(path_secret_map_id_entry_evicted__peer_address__protocol),
                    270usize => Self(path_secret_map_address_entry_evicted__peer_address__protocol),
                    273usize => Self(unknown_path_secret_packet_sent__peer_address__protocol),
                    275usize => Self(unknown_path_secret_packet_received__peer_address__protocol),
                    277usize => Self(unknown_path_secret_packet_accepted__peer_address__protocol),
                    279usize => Self(unknown_path_secret_packet_rejected__peer_address__protocol),
                    281usize => Self(unknown_path_secret_packet_dropped__peer_address__protocol),
                    289usize => Self(replay_detected_packet_sent__peer_address__protocol),
                    291usize => Self(replay_detected_packet_received__peer_address__protocol),
                    293usize => Self(replay_detected_packet_accepted__peer_address__protocol),
                    295usize => Self(replay_detected_packet_rejected__peer_address__protocol),
                    297usize => Self(replay_detected_packet_dropped__peer_address__protocol),
                    299usize => Self(stale_key_packet_sent__peer_address__protocol),
                    301usize => Self(stale_key_packet_received__peer_address__protocol),
                    303usize => Self(stale_key_packet_accepted__peer_address__protocol),
                    305usize => Self(stale_key_packet_rejected__peer_address__protocol),
                    307usize => Self(stale_key_packet_dropped__peer_address__protocol),
                    309usize => {
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                186usize => Self(stream_packet_abandoned__packet_len),
                188usize => Self(stream_packet_abandoned__payload_len),
                189usize => Self(stream_packet_abandoned__payload_len__conn),
                190usize => Self(stream_packet_abandoned__lifetime),
                193usize => Self(stream_packet_spuriously_retransmitted__packet_len),
                195usize => Self(stream_packet_spuriously_retransmitted__payload_len),
                196usize => Self(stream_packet_spuriously_retransmitted__payload_len__conn),
                200usize => Self(stream_max_data_received__increase),
                204usize => Self(stream_control_packet_transmitted__packet_len),
                205usize => Self(stream_control_packet_transmitted__control_data_len),
                207usize => Self(stream_control_packet_received__packet_len),
                208usize => Self(stream_control_packet_received__control_data_len),
                220usize => Self(endpoint_udp_immediate_transmission_scheduled__buffer_size),
                221usize => Self(endpoint_udp_immediate_transmission_scheduled__segment_size),
                222usize => Self(endpoint_udp_immediate_transmission_scheduled__segment_count),
                225usize => Self(endpoint_udp_transmission_scheduled__buffer_size),
                226usize => Self(endpoint_udp_transmission_scheduled__segment_size),
                227usize => Self(endpoint_udp_transmission_scheduled__segment_count),
                228usize => Self(endpoint_udp_transmission_scheduled__delay),
                231usize => Self(endpoint_udp_transmission_rejected__buffer_size),
                232usize => Self(endpoint_udp_transmission_rejected__segment_size),
                233usize => Self(endpoint_udp_transmission_rejected__segment_count),
                234usize => Self(endpoint_udp_transmission_rejected__delay),
                235usize => Self(endpoint_udp_transmission_rejected__backoff),
                238usize => Self(endpoint_udp_packet_transmitted__buffer_size),
                239usize => Self(endpoint_udp_packet_transmitted__segment_size),
                240usize => Self(endpoint_udp_packet_transmitted__segment_count),
                243usize => Self(endpoint_udp_transmit_errored__buffer_size),
                244usize => Self(endpoint_udp_transmit_errored__segment_size),
                245usize => Self(endpoint_udp_transmit_errored__segment_count),
                248usize => Self(endpoint_udp_packet_received__buffer_size),
                249usize => Self(endpoint_udp_packet_received__segment_size),
                250usize => Self(endpoint_udp_packet_received__segment_count),
                253usize => Self(path_secret_map_initialized__capacity),
                255usize => Self(path_secret_map_uninitialized__capacity),
                256usize => Self(path_secret_map_uninitialized__entries),
                257usize => Self(path_secret_map_uninitialized__lifetime),
                268usize => Self(path_secret_map_id_entry_evicted__age),
                271usize => Self(path_secret_map_address_entry_evicted__age),
                283usize => Self(key_accepted__gap),
                284usize => Self(key_accepted__forward_shift),
                287usize => Self(replay_potentially_detected__gap),
                313usize => Self(path_secret_map_address_cache_accessed_hit__age),
                317usize => Self(path_secret_map_id_cache_accessed_hit__age),
                319usize => Self(path_secret_map_cleaner_cycled__entries__id),
                320usize => Self(path_secret_map_cleaner_cycled__entries__id__retired),
                321usize => Self(path_secret_map_cleaner_cycled__entries__id__active),
                322usize => Self(path_secret_map_cleaner_cycled__entries__id__active__utilization),
                323usize => Self(path_secret_map_cleaner_cycled__entries__id__utilization),
                324usize => Self(path_secret_map_cleaner_cycled__entries__id__utilization__initial),
                325usize => Self(path_secret_map_cleaner_cycled__entries__address),
                326usize => Self(path_secret_map_cleaner_cycled__entries__address__active),
                327usize => {
                    Self(path_secret_map_cleaner_cycled__entries__address__active__utilization)
                }
                328usize => Self(path_secret_map_cleaner_cycled__entries__address__retired),
                329usize => Self(path_secret_map_cleaner_cycled__entries__address__utilization),
                330usize => {
                    Self(path_secret_map_cleaner_cycled__entries__address__utilization__initial)
                }
                331usize => Self(path_secret_map_cleaner_cycled__handshake_requests),
                332usize => Self(path_secret_map_cleaner_cycled__handshake_requests__retired),
                333usize => Self(path_secret_map_cleaner_cycled__handshake_lock_duration),
                334usize => Self(path_secret_map_cleaner_cycled__total_duration),
                336usize => Self(path_secret_map_id_write_lock__acquire),
                337usize => Self(path_secret_map_id_write_lock__duration),
                339usize => Self(path_secret_map_address_write_lock__acquire),
                340usize => Self(path_secret_map_address_write_lock__duration),
                _ => unreachable!("invalid info: {info:?}"),
            }
        }
    }
