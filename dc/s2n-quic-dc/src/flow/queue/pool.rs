    ...
    Key: 'static + Send + Sync,
{
    ...
    fn grow(&mut self) {
        ...
        for idx in 0..PAGE_SIZE {
            // Give the descriptor a reference to the free list so deallocated receiver
            // pairs are recycled back into this pool.
            let free_list = self.free.clone();
            let descriptor = Descriptor::new(self.epoch + idx, free_list);
        ...
        self.free.record_region(pending_desc);
    }
}
