...
/// write_msg yields cooperatively after BUDGET consecutive completions, giving
/// other tasks a chance to run even under continuous write pressure.
///
/// This test verifies the outer `coop::poll` budget mechanism: after 128
/// successful write_msg completions, the writer schedules a wake-up and returns
/// Pending, allowing a competing task to execute.
#[test]
fn write_msg_coop_yields_after_budget_completions() {
    // coop::BUDGET is 128; use one more to guarantee at least one yield.
    const WRITES: usize = 129;
