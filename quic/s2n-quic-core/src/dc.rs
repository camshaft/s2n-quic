...
pub struct ConnectionInfo<'a> {
    ...
    /// transport parameter. The encoding is opaque to core; higher layers
    /// (e.g. `s2n-quic-dc`) are responsible for decoding.
    #[cfg(feature = "alloc")]
    pub peer_data_addrs: Option<bytes::Bytes>,
}
impl<'a> ConnectionInfo<'a> {
    ...
    pub fn new(
        ...
        peer_data_addrs: Option<bytes::Bytes>,
    ) -> Self {
        Self {
...
pub struct ApplicationParams {
    ...
    /// Maximum number of queue slots the peer can address.
    pub max_queues: VarInt,
}
...
decoder_value!(
    impl<'a> ApplicationParams {
        fn decode(buffer: Buffer) -> Result<Self> {
            ...
            let (max_queues, buffer) = buffer.decode::<VarInt>()?;
